test_that("getBeta returns big.matrix when size exceeds threshold with file input", {
    ...
    options("DMRsegal.subset_beta_as_bigmem_mb" = 0)
    ...
    result <- beta_handler$getBeta(row_names = row_names_subset, check_mem = TRUE)
...
test_that("getBeta returns regular matrix when size below threshold", {
    ...
    beta_df <- cbind(CpG = rownames(beta), as.data.frame(beta))
    write.table(beta_df, temp_file, sep = "\t", row.names = FALSE, quote = FALSE)
    options("DMRsegal.subset_beta_as_bigmem_mb" = 100)
    beta_handler <- getBetaHandler(
        beta = temp_file,
        array = "450K",
        genome = "hg19"
...
test_that("getBeta big.matrix has correct values", {
    ...
    result_big <- beta_handler$getBeta(row_names = row_names_subset, check_mem = TRUE)
    options("DMRsegal.beta_in_mem_threshold_mb" = 100)
    options("DMRsegal.subset_beta_as_bigmem_mb" = 100)
    ...
    result_normal <- beta_handler_normal$getBeta(row_names = row_names_subset, check_mem = TRUE)
...
test_that("getBeta big.matrix with column subset", {
    ...
    result <- beta_handler$getBeta(row_names = row_names_subset, col_names = col_names_subset, check_mem = TRUE)
...
test_that("getBeta big.matrix handles all rows", {
    ...
    result <- beta_handler$getBeta(check_mem = TRUE)
