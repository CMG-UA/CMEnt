...
## Performance of Scoring across Methods
```{r}
pheno <- as.data.frame(pheno)
rownames(pheno) <- pheno$V1
scored_dmrs_file <- getBenchmarkFile("scored_dmrs.rds")
if (!file.exists(scored_dmrs_file)) {
    scored_methods_list <- list()
    for (method in names(methods_list)) {
        dmrs <- methods_list[[method]]
        if ("score" %in% names(mcols(dmrs))) {
            next
        }
        cat(sprintf("Scoring DMRs for method: %s\n", method))
        scored_methods_list[[method]] <- CMEnt::scoreDMRs(
            dmrs=dmrs, beta=beta_handler, pheno=pheno,
            sample_group_col = sample_group_col,
            array = array_type, genome = genome,
            njobs = min(8, parallel::detectCores() - 1),
            verbose = 3
        )
    }
    saveRDS(scored_methods_list, scored_dmrs_file)
} else {
    scored_methods_list <- readRDS(scored_dmrs_file)
}

# Report top scoring DMR for each method
top_scoring_tbl <- do.call(rbind, lapply(names(scored_methods_list), function(method) {
    dmrs <- scored_methods_list[[method]]
    if (!"score" %in% names(mcols(dmrs)) || length(dmrs) == 0) {
        return(data.frame(Method = method, Top_Score = NA))
    }
    data.frame(Method = method, Top_Score = max(mcols(dmrs)$score, na.rm = TRUE))
}))
knitr::kable(top_scoring_tbl, digits = 4, caption = "Top DMR Score by Method")
```
## DMR Interactions Circos visualization
