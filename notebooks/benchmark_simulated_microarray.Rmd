...
if (!file.exists(sim_beta_path)) {
    ...
    # )
    controls <- get_benchmark_file("controls.tsv")
    set.seed(42)
    ret <- CMEnt::simulateDMRs(controls, num_dmrs = 500, genome = genome, array = array_type)
    ret$simulated <- round(ret$simulated, 5)
    data.table::fwrite(as.data.frame(ret$simulated), sim_beta_path, sep = "\t", quote = FALSE, row.names = TRUE, col.names = TRUE)
...
# Correct using lag and lead p-values to account for spatial correlation in microarray data
dmps$pval_adj <- CMEnt::combinePvalues(
    pvals = dmps$pval,
    positions = dmp_locs$start,
    max_dist = 500,
    method = "fdr"
)
...
gt_seeds_gr_21 <- subsetByOverlaps(seeds_gr_21, truth_dmrs, ignore.strand = TRUE)
gt_seeds_21 <- mcols(gt_seeds_gr_21)$id
seeds_beta_21 <- beta_mat[mcols(gt_seeds_gr_21)$id, , drop = FALSE]
seeds_dmrs_cment <- CMEnt::findDMRsFromSeeds(
    beta = seeds_beta_21,
    seeds = gt_seeds_21,
    pheno = pheno,
    sample_group_col = "Sample_Group",
    ignored_sample_groups  = "control",
    casecontrol_col = "casecontrol",
    testing_mode  = "parametric",
    max_bridge_seeds_gaps = 0,
    max_bridge_extension_gaps = 0,
    min_seeds = 2,
    min_adj_seeds = cment_params$min_adj_seeds,
    min_sites = 2,
    ext_site_delta_beta = NA,
