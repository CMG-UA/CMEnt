...
get_method_cache_file <- function(method, genome) {
    get_benchmark_file(sprintf("dmrs.%s.%s.rds", method, genome))
}
```
...
```{r CMEnt}
cment_file <- get_method_cache_file("CMEnt", genome)
...
```{r comb-p}
combp_file <- get_method_cache_file("combp", genome)
...
## Method 3: probeLasso
```{r probeLasso}
probelasso_file <- get_method_cache_file("probeLasso", genome)
...
```{r dmrcate}
dmrcate_file <- get_method_cache_file("dmrcate", genome)
...
```{r bumphunter}
bumphunter_file <- get_method_cache_file("bumphunter", genome)
