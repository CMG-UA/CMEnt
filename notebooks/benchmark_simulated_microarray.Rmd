...
get_method_cache_file <- function(method) {
    get_benchmark_file(sprintf("dmrs.%s.rds", method))
}
```
...
cment_params <- list(
    ...
    min_seeds = 2,
    min_sites = 5,
    # The simulated truth interval uses truth_min_delta_beta = 0.05.
    ext_site_delta_beta = 0.05,
...
if (!cment_cache_valid) {
    ...
    dmrs_cment <- CMEnt::findDMRsFromSeeds(
        ...
        max_bridge_seeds_gaps = cment_params$max_bridge_seeds_gaps,
        max_bridge_extension_gaps = cment_params$max_bridge_extension_gaps,
        min_seeds = cment_params$min_seeds,
        min_sites = cment_params$min_sites,
        ext_site_delta_beta = cment_params$ext_site_delta_beta,
    ...
    dmrs_dmrcate <- tryCatch(
        {
            if (ncol(design) < 2L) {
                ...
                extractRanges(dmrcate_results, genome = genome)
            }
        },
        error = function(e) {
...
annotate_dmrs <- function(dmrs, method_name) {
    ...
    annotated_dmrs <- tryCatch(
        suppressWarnings(CMEnt::annotateDMRsWithGenes(dmrs, genome = genome)),
    ...
    promoter_hits <- !is.na(mcols(annotated_dmrs)$in_promoter_of) & nzchar(mcols(annotated_dmrs)$in_promoter_of)
    gene_hits <- !is.na(mcols(annotated_dmrs)$in_gene_body_of) & nzchar(mcols(annotated_dmrs)$in_gene_body_of)
    intergenic <- sum(!(promoter_hits | gene_hits))

    data.frame(
        Method = method_name,
        Total_DMRs = length(annotated_dmrs),
        In_Promoters = sum(promoter_hits),
        In_Gene_Bodies = sum(gene_hits),
        Intergenic = intergenic,
        Pct_Promoters = round(100 * sum(promoter_hits) / length(annotated_dmrs), 1),
        Pct_Gene_Bodies = round(100 * sum(gene_hits) / length(annotated_dmrs), 1),
        Pct_Intergenic = round(100 * intergenic / length(annotated_dmrs), 1)
    )
}

gene_annotation_list <- Filter(Negate(is.null), lapply(names(methods_list), function(method) {
    annotate_dmrs(methods_list[[method]], method)
}))

if (length(gene_annotation_list) > 0) {
...
if (requireNamespace("IlluminaHumanMethylation450kanno.ilmn12.hg19", quietly = TRUE)) {
    ...
    common_sites <- intersect(rownames(locs), rownames(anno))

    site_context_gr <- makeGRangesFromDataFrame(
        data.frame(
            chr = locs[common_sites, "chr"],
            start = locs[common_sites, "start"],
            end = locs[common_sites, "end"],
            context = anno[common_sites, "Relation_to_Island"]
        ),
        keep.extra.columns = TRUE
    )
    names(site_context_gr) <- common_sites
...
mcols(dmps_gr)$index <- seq_along(dmps_gr)
seeds_in_truth <- subsetByOverlaps(dmps_gr, truth_dmrs, ignore.strand = TRUE)

if (length(dmrs_cment) > 0) {
    seeds_in_dmrs <- subsetByOverlaps(dmps_gr, dmrs_cment, ignore.strand = TRUE)
    ...
    seeds_in_dmrs <- dmps_gr[0]
    seeds_in_truth_and_dmrs <- dmps_gr[0]
