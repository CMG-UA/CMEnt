...
The dataset used is a simulated dataset, produced by running `simulateDMRs`, on a chr20 and chr21 subset of preprocessed beta values from 100 randomly selected 450K illumina microarray healthy blood samples found in GEO with identifier GSE40279, with 500 simulated DMRs of varying widths and effect sizes (50 cases and 50 controls). The simulated dataset is available for download from Zenodo (see code below).
...
if (!is.null(ret)) {
    ...
    has_informative_bh_null <- function(fit) {
        ...
        nrow(bumphunter_results$table) > 0 &&
            (
                all(is.na(bumphunter_results$table$p.value)) ||
                    !has_informative_bh_null(bumphunter_results)
            ) &&
...
stats_list <- list(
    ...
    get_dmr_stats(truth_dmrs, "Truth")
)
