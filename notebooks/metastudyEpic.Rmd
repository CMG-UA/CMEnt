...
ggplot(dmr_effect_df, aes(x = Delta_beta, y = Width, color = Direction)) +
    geom_hline(
        yintercept = median(dmr_effect_df$Width[dmr_effect_df$Delta_beta > 0], na.rm = TRUE),
        linewidth = 0.2,
        color = "#B8405E"
    ) +
    geom_hline(
...
top_10_cment_dmrs <- as.data.frame(dmrs_cment) |>
    arrange(by = -score) |>
    head(10) |>
    dplyr::select(id, score, in_promoter_of, in_gene_body_of)
knitr::kable(
    top_10_cment_dmrs,
    digits = 3,
    row.names = FALSE,
...
if (file.exists(cached)) {
    ...
    if (!"PathwayID" %in% colnames(missmethyl_long)) {
        unlink(cached)
        missmethyl_long <- NULL
    }
}
if (is.null(missmethyl_long)) {
    missmethyl_long <- base::do.call(rbind, Filter(Negate(is.null), unlist(lapply(names(methods_list), function(method) {
        lapply(c("GO", "KEGG"), function(collection) {
            cat(sprintf("Running missMethyl %s enrichment for %s...\n", collection, method))
            ret <- tryCatch(
                missMethyl::goregion(
...
if (!is.null(missmethyl_long) && nrow(missmethyl_long) > 0L &&
    ...
    promoter_gene_delta <- as.data.frame(dmrs_cment) |>
        arrange(desc(score)) |>
        slice_head(n = min(100L, length(dmrs_cment))) |>
    ...
    if (nrow(kegg_candidates) > 0L && nrow(promoter_gene_delta) > 0L) {
        ...
        if (nrow(promoter_gene_delta) == 0L) {
            ...
            if (is.na(matched_pathway)) {
                ...
                if (file.exists(basename(pathview_png)) && !file.exists(pathview_png)) {
