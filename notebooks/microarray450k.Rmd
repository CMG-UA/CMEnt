...
if (exists("site_context_df")) {
    ...
    diff_df <- plot_context_df |>
        select(Source, Relation_to_Island, Sites) |>
    ...
    ggplot(plot_context_df, aes(x = Sites, y = Relation_to_Island, fill = Source)) +
        geom_col(data = filter(plot_context_df, Source == "Significant DMPs"), width = 0.78, alpha = 0.45) +
        geom_col(data = filter(plot_context_df, Source == "Sites inside CMEnt DMRs"), width = 0.48, alpha = 0.9) +
