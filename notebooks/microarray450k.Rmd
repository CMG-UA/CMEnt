...
dmr_mood <- dmr_summary %>%
  group_by(Region) %>%
  summarise(star_y = exp(mean(log(median_width_bp))), .groups = "drop") %>%
  left_join(
    dmr_effect_df %>%
      group_by(Region) %>%
  ...
  mutate(
    ...
    star_x = ifelse(Region == region_levels[1], x_range[1], x_range[2]),
    hjust = ifelse(Region == region_levels[1], 0, 1)
  )
