---
title: "DMRsegal vs DMRcate vs dmrseq vs hummingbird on Simulated ONT BSseq Data"
...
output:
    BiocStyle::html_document:
        code_folding: hide
vignette: >
  %\VignetteIndexEntry{DMRsegal vs DMRcate vs dmrseq vs hummingbird on Simulated ONT BSseq Data}
...
if (!file.exists(sim_bs_path)) {
    curl::curl_download(
        url = "https://zenodo.org/records/19442263/files/sim_bs_1000.rds?download=1",
        destfile = sim_bs_path
    )
}
if (!file.exists(sim_truth_path)) {
    curl::curl_download(
        url = "https://zenodo.org/records/19442263/files/sim_dmrs_1000.tsv?download=1",
...
We extend beyond strict FDR significance to capture a broader set of potential seeds, 
which may improve sensitivity in DMRsegal's region assembly.
We score seeds by a combined score of p-value and effect size (delta beta) to prioritize biologically relevant candidates.
...
seeds_meta_rds <- get_benchmark_file("seeds.meta.rds")
if (!file.exists(seeds_tsv)) {
    ...
    out <- gzfile(seeds_tsv, "w")
    write.table(sig_seeds, out, sep = "\t", quote = FALSE, row.names = FALSE)
    close(out)
...
seed_locs <- seeds_meta$seed_locs
sig_seeds <- as.data.frame(sig_seeds, stringsAsFactors = FALSE)
rownames(sig_seeds) <- sig_seeds$cpg_id
sig_seeds <- merge(sig_seeds, seed_locs, by = 0)
sig_seeds$Row.names <- NULL
...
cat(sprintf("Number of seeds for DMRsegal: %d\n", nrow(sig_seeds)))
knitr::kable(
    seeds_qs,
    digits = 2,
...
- Seeds_Total: Total number of seed CpGs identified.
- Seeds_Overlapping_Truth: Number of seed CpGs that overlap any ground-truth DMR.
- Seeds_Overlap_Pct: Percentage of seed CpGs that overlap ground-truth DMRs.
- Truth_DMRs_Total: Total number of ground-truth DMRs.
- Truth_DMRs_With_Seed_Pct: Percentage of ground-truth DMRs that have at least one overlapping seed CpG.
- Truth_DMRs_With_2Seeds_Pct: Percentage of ground-truth DMRs that have at least two overlapping seed CpGs.
...
if (!file.exists(dmrsegal_file)) {
    ...
    dmrs_segal <- progressr::with_progress(
        DMRsegal::findDMRsFromSeeds(
            ...
            min_seeds = 2,
            min_cpgs = 2,
            min_cpg_delta_beta = 0.05, # Require a minimum effect size for CpGs to be included in DMRs, which can help filter out very small changes
            max_lookup_dist = 100000, # Use a larger lookup distance (10k is the default)
            chunk_size = 1e6L,
            entanglement = "weak",
...
run_hummingbird_bsseq <- function(
    ...
    meth_mat <- bsseq::getCoverage(bs_obj, type = "M")
    cov_mat <- bsseq::getCoverage(bs_obj, type = "Cov")
    ...
    for (chr_name in chr_vec) {
        chr_idx <- which(all_seqnames == chr_name)
        if (length(chr_idx) < as.integer(min_cpgs)) {
            next
        }
        ...
        control_se <- SummarizedExperiment::SummarizedExperiment(
            rowRanges = chr_gr,
            assays = list(
                normM = to_count_matrix(meth_mat[chr_idx, ctrl_idx, drop = FALSE]),
                normUM = to_count_matrix(unmeth_mat[chr_idx, ctrl_idx, drop = FALSE])
            )
        )
        case_se <- SummarizedExperiment::SummarizedExperiment(
            rowRanges = chr_gr,
            assays = list(
                abnormM = to_count_matrix(meth_mat[chr_idx, case_idx, drop = FALSE]),
                abnormUM = to_count_matrix(unmeth_mat[chr_idx, case_idx, drop = FALSE])
            )
        )
        ...
        rm(chr_gr, control_se, case_se, chr_out)
        gc(verbose = FALSE)
    }
...
### DMRsegal Sensitivity Analysis

This section quantifies how much of the ONT performance gap can be reduced by relaxing
DMRsegal's seed/extension constraints. It is intentionally optional because each rerun can
be time-consuming on the full 8M+ CpG dataset.

- Set `options(DMRsegal.run_sensitivity = TRUE)` before rendering to execute reruns.
- Results are cached in `inst/benchmarks/simulated_ont/dmrs.DMRsegal.sensitivity.rds`.

```{r dmrsegal_sensitivity}
run_sensitivity <- isTRUE(getOption("DMRsegal.run_sensitivity", FALSE))
sensitivity_file <- get_benchmark_file("dmrs.DMRsegal.sensitivity.rds")

run_dmrsegal_variant <- function(param_overrides = list()) {
    dmrsegal_njobs <- max(1L, min(8L, future::availableCores() - 1L))
    options("DMRsegal.njobs" = dmrsegal_njobs)
    options("DMRsegal.verbose" = 1)

    beta_handler <- DMRsegal::getBetaHandler(beta = sim_bs)
    base_args <- list(
        beta = beta_handler,
        seeds = sig_seeds,
        pheno = pheno,
    ...
    call_args <- utils::modifyList(base_args, param_overrides)
    start_time <- Sys.time()
    dmrs_out <- progressr::with_progress(
        do.call(DMRsegal::findDMRsFromSeeds, call_args)
    )
    elapsed <- Sys.time() - start_time
    if (is.null(dmrs_out)) {
        dmrs_out <- GenomicRanges::GRanges()
    }
    list(dmrs = dmrs_out, time = elapsed)
}

format_variant_params <- function(x) {
    if (length(x) == 0) {
        return("same as benchmark DMRsegal run")
    }
    paste(
        sprintf(
            "%s=%s",
...
scenario_defs <- list(
    Baseline_cached = list(
        params = list()
    ),
    FixedDelta_0.10 = list(
        params = list(
            min_cpg_delta_beta = 0.10
        )
    ),
    FixedDelta_0.05 = list(
        params = list(
            min_cpg_delta_beta = 0.05
        )
    ),
    RelaxedConnectivity = list(
        params = list(
            max_pval = 0.2,
            min_seeds = 1,
            min_cpgs = 2,
            max_bridge_seeds_gaps = 2L,
            max_bridge_extension_gaps = 6L,
            expansion_window = 20000,
            chunk_size = 2e5L,
            njobs = 1L
        )
    ),
    CombinedRelaxed = list(
        params = list(
            min_cpg_delta_beta = 0.05,
            max_pval = 0.2,
            min_seeds = 1,
...
if (file.exists(sensitivity_file)) {
    sensitivity_ret <- readRDS(sensitivity_file)
} else if (run_sensitivity) {
    scenario_runs <- vector("list", length(scenario_defs))
    names(scenario_runs) <- names(scenario_defs)

    for (scenario_name in names(scenario_defs)) {
        if (scenario_name == "Baseline_cached") {
            scenario_runs[[scenario_name]] <- list(dmrs = dmrs_segal, time = time_dmrsegal)
            next
        }
        cat(sprintf("Running sensitivity scenario: %s\n", scenario_name))
        scenario_runs[[scenario_name]] <- run_dmrsegal_variant(
            param_overrides = scenario_defs[[scenario_name]]$params
        )
    }

    sensitivity_ret <- list(
        scenarios = scenario_runs,
        defs = scenario_defs,
        generated_on = Sys.time()
    )
    saveRDS(sensitivity_ret, sensitivity_file)
} else {
    sensitivity_ret <- NULL
}

if (is.null(sensitivity_ret)) {
    cat("Sensitivity reruns were not executed.\n")
    cat("Set options(DMRsegal.run_sensitivity = TRUE) and re-render to compute them.\n")
} else {
    if (is.null(sensitivity_ret$defs)) {
        sensitivity_ret$defs <- scenario_defs
    }

    sensitivity_eval <- do.call(rbind, lapply(names(sensitivity_ret$scenarios), function(scenario_name) {
        ret <- sensitivity_ret$scenarios[[scenario_name]]
        def_entry <- sensitivity_ret$defs[[scenario_name]]
        params_entry <- if (!is.null(def_entry) && !is.null(def_entry$params)) def_entry$params else list()
        dmrs_obj <- ret$dmrs
        if (is.null(dmrs_obj)) {
            dmrs_obj <- GenomicRanges::GRanges()
        }
        out <- evaluate_against_truth(dmrs_obj, truth_dmrs)
        out$Scenario <- scenario_name
        out$N_DMRs <- length(dmrs_obj)
        out$Median_Width_bp <- ifelse(length(dmrs_obj) == 0, NA_real_, stats::median(width(dmrs_obj)))
        out$Elapsed_sec <- as_elapsed_seconds(ret$time)
        out$Params <- format_variant_params(params_entry)
        out
    }))

    dmrseq_ref <- evaluate_against_truth(dmrs_dmrseq, truth_dmrs)
    dmrseq_ref$Scenario <- "dmrseq_reference"
    dmrseq_ref$N_DMRs <- length(dmrs_dmrseq)
    dmrseq_ref$Median_Width_bp <- ifelse(length(dmrs_dmrseq) == 0, NA_real_, stats::median(width(dmrs_dmrseq)))
    dmrseq_ref$Elapsed_sec <- as_elapsed_seconds(time_dmrseq)
    dmrseq_ref$Params <- "dmrseq::dmrseq reference run"

    sensitivity_eval <- rbind(sensitivity_eval, dmrseq_ref)

    baseline_idx <- which(sensitivity_eval$Scenario == "Baseline_cached")
    baseline_f1_bp <- if (length(baseline_idx) == 1) sensitivity_eval$F1_bp[baseline_idx] else NA_real_
    baseline_f1_regions <- if (length(baseline_idx) == 1) sensitivity_eval$F1_regions[baseline_idx] else NA_real_
    sensitivity_eval$Delta_F1_bp_vs_Baseline <- sensitivity_eval$F1_bp - baseline_f1_bp
    sensitivity_eval$Delta_F1_regions_vs_Baseline <- sensitivity_eval$F1_regions - baseline_f1_regions

    sensitivity_table <- sensitivity_eval |>
        dplyr::select(
            Scenario,
            N_DMRs,
            Precision_regions,
            ...
            F1_bp,
            IoU_bp,
            Delta_F1_bp_vs_Baseline,
            Median_Width_bp,
            Elapsed_sec,
            Params
        ) |>
        dplyr::arrange(dplyr::desc(F1_bp))

    knitr::kable(
        sensitivity_table,
        digits = 4,
        caption = "DMRsegal sensitivity grid (plus dmrseq reference)"
    )
}
```
...
```{r}
if (!is.null(sensitivity_ret)) {
    dmrseq_ref_f1 <- sensitivity_table$F1_bp[sensitivity_table$Scenario == "dmrseq_reference"]
    plot_df <- sensitivity_table[sensitivity_table$Scenario != "dmrseq_reference", , drop = FALSE]
    plot_df$Scenario <- factor(plot_df$Scenario, levels = plot_df$Scenario[order(plot_df$F1_bp)])

    ggplot(plot_df, aes(x = Scenario, y = F1_bp, fill = Scenario)) +
        geom_col() +
        geom_hline(
            yintercept = dmrseq_ref_f1,
            linetype = "dashed",
            linewidth = 0.7,
            color = "black"
        ) +
        ...
        ylim(0, 1) +
        labs(
            title = "DMRsegal ONT sensitivity: base-pair F1",
            subtitle = "Dashed line shows dmrseq reference F1",
            x = "Scenario",
            y = "F1 (base-pair)"
        )
}
...
- Truth_DMRs_Total: Total number of ground truth DMRs.
- Truth_DMRs_With_Seeds: Number of truth DMRs that have at least one overlapping seed CpG.
- Truth_DMRs_Stage1Recoverable_2plusSeeds: Number of truth DMRs that have at least 2 overlapping seed CpGs, which is the minimum for DMRsegal stage 1 to potentially recover the region.
- Truth_DMRs_Stage1Recoverable_With2plusCapturedSeeds: Number of truth DMRs that have at least 2 overlapping seed CpGs that also overlap DMRsegal-predicted DMRs, indicating they are potentially recoverable and captured by the method.
- Truth_DMRs_Stage1Recoverable_FullyCapturedSeeds: Number of truth DMRs where all overlapping seed CpGs are captured by DMRsegal-predicted DMRs, indicating ideal stage 1 capture.
- Seed_Capture_Rate_Pct_in_Truth: Percentage of seed CpGs that overlap truth DMRs that are also captured by DMRsegal-predicted DMRs.
- Median_Seed_Capture_Pct_Per_Truth: For truth DMRs that have at least one overlapping seed CpG, the median percentage of those seed CpGs that are captured by DMRsegal-predicted DMRs.
- Truth_DMRs_With_Missed_Seeds: Number of truth DMRs that have at least one overlapping seed CpG that is not captured by DMRsegal-predicted DMRs.
- Median_Max_Missed_Run_Per_Truth: For truth DMRs with missed seeds, the median length of the longest consecutive run of missed seed CpGs.
- Max_Missed_Run_Observed: The maximum length of consecutive missed seed CpGs observed across all truth DMRs.
...
- Pred_DMRs_Total: Total number of DMRsegal DMRs.
- Pred_DMRs_Overlapping_Truth: Number of DMRsegal DMRs that overlap at least one truth DMR.
- Pred_DMRs_BPPrecision_ge50pct: Number of DMRsegal DMRs with base-pair precision >= 50% (i.e., at least 50% of the DMRsegal DMR's base pairs overlap with truth DMRs).
- Pred_DMRs_BPPrecision_ge80pct: Number of DMRsegal DMRs with base-pair precision >= 80%.
- Truth_DMRs_Total: Total number of truth DMRs.
- Truth_DMRs_Hit: Number of truth DMRs that have at least one overlapping DMRsegal DMR.
- Truth_DMRs_BPRecall_ge50pct: Number of truth DMRs with base-pair recall >= 50% (i.e., at least 50% of the truth DMR's base pairs are covered by DMRsegal DMRs).
- Truth_DMRs_BPRecall_ge80pct: Number of truth DMRs with base-pair recall >= 80%.
- Median_Truth_BPRecall_Pct_Hit: Median base-pair recall percentage among truth DMRs that have at least one overlapping DMRsegal DMR.
- Median_Pred_BPPrecision_Pct_Overlap: Median base-pair precision percentage among DMRsegal DMRs that have at least some overlap with truth DMRs.
- Truth_DMRs_With_Multiple_Pred: Number of truth DMRs that have 2 or more overlapping DMRsegal DMRs, indicating potential fragmentation.
- Median_Preds_Per_Hit_Truth: Median number of overlapping DMRsegal DMRs among truth DMRs that have at least one overlapping DMRsegal DMR.
- Max_Preds_On_Truth: Maximum number of overlapping DMRsegal DMRs observed on any single truth DMR.
...
DMRsegal DMR multiplicity per truth DMR.
Point color encodes truth-side base-pair recall (%)
...
dmrs_in_truth_counts_df[dmrs_in_truth_counts_df[, "Truth_BP_Recall_Pct"] == 0, "Truth_BP_Recall_Pct"] <- NA
dmrs_points_df <- dmrs_in_truth_counts_df[
    dmrs_in_truth_counts_df$DMRsegal_DMR_Count > 0,
    ,
    drop = FALSE
]
color_upper <- as.numeric(stats::quantile(
    dmrs_points_df$Truth_BP_Recall_Pct,
    probs = 0.95,
    na.rm = TRUE,
    names = FALSE
))
color_lower <- suppressWarnings(min(dmrs_points_df$Truth_BP_Recall_Pct, na.rm = TRUE))
if (!is.finite(color_lower) || !is.finite(color_upper)) {
    color_lower <- 0
    color_upper <- 1
}
if (color_upper <= color_lower) {
    color_upper <- color_lower + 1e-6
}

ggplot(dmrs_in_truth_counts_df,
    aes(x = Truth_DMR_Index, y = DMRsegal_DMR_Count, color = Truth_BP_Recall_Pct)
    ) +
    scale_y_continuous(breaks = scales::breaks_pretty()) +
    geom_point(data = dmrs_points_df, size = 0.6, alpha = 0.7, na.rm = TRUE) +
    scale_color_viridis_c(
        option = "C",
        limits = c(color_lower, color_upper),
        oob = scales::squish
    ) +
    theme_minimal() +
    theme(
