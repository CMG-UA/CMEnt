...
drop_zero_coverage_cpgs <- function(bs_obj) {
    cov_assay <- SummarizedExperiment::assay(bs_obj, "Cov", withDimnames = FALSE)
    valid_cpgs <- DelayedMatrixStats::rowSums2(cov_assay) > 0
    rm(cov_assay)
    gc(verbose = FALSE)
    if (!all(valid_cpgs)) {
        bs_obj <- bs_obj[valid_cpgs, ]
    }
    rm(valid_cpgs)
    gc(verbose = FALSE)
    bs_obj
...
is_hdf5_bsseq <- function(bs_obj) {
    if (!inherits(bs_obj, "BSseq")) {
        return(FALSE)
    }
    assay_classes <- vapply(
        SummarizedExperiment::assays(bs_obj, withDimnames = FALSE),
        function(x) inherits(x, "HDF5Matrix"),
        logical(1)
    )
    all(c("M", "Cov") %in% names(assay_classes)) && all(assay_classes[c("M", "Cov")])
}

get_hdf5_bsseq <- function(input_rds, hdf5_dir) {
    if (dir.exists(hdf5_dir)) {
        bs_obj <- tryCatch(
            HDF5Array::loadHDF5SummarizedExperiment(hdf5_dir),
            error = function(e) NULL
        )
        if (is_hdf5_bsseq(bs_obj)) {
            return(bs_obj)
        }
    }
    ...
    dir.create(hdf5_dir, showWarnings = FALSE, recursive = TRUE)

    bs_obj <- readRDS(input_rds)
    stopifnot(inherits(bs_obj, "BSseq"))
    bs_obj <- drop_zero_coverage_cpgs(bs_obj)

    chunkdim <- c(min(100000L, nrow(bs_obj)), ncol(bs_obj))
    hdf5_bs <- HDF5Array::saveHDF5SummarizedExperiment(
        bs_obj,
        dir = hdf5_dir,
        replace = TRUE,
        chunkdim = chunkdim,
...
sim_bs_hdf5_dir <- get_benchmark_file("sim_bs_1000_hdf5")
...
if (computation_needed) {
    sim_bs <- get_hdf5_bsseq(sim_bs_path, sim_bs_hdf5_dir)
} else {
...
max_lookup_dist <- 10000L
```

#### Extract Methylation Matrix and Generate Seeds
This step is crucial for CMEnt, which relies on seed CpGs to guide DMR assembly.
...
- Truth_DMRs_With_Enough_Seeds_Pct: Percentage of ground-truth DMRs that have enough overlapping seed CpGs to be captured by CMEnt.
...
if (!file.exists(cment_file)) {
    ...
    options("CMEnt.parallel_backend" = "multisession")
    ...
    rm(sim_bs)
    gc(verbose = FALSE)
    dmrs_cment <- progressr::with_progress(
        CMEnt::findDMRsFromSeeds(
            beta = beta_handler,
            seeds = seeds,
            pheno = pheno,
            seeds_id_col = "cpg_id",
...
- Truth_DMRs_Stage1Recoverable_2plusSeeds: Number of truth DMRs that have at least 2 overlapping seed CpGs, which is the minimum for CMEnt stage 1 to potentially recover the region.
- Truth_DMRs_Stage1Recoverable_With2plusCapturedSeeds: Number of truth DMRs that have at least 2 overlapping seed CpGs that also overlap CMEnt-predicted DMRs, indicating they are potentially recoverable and captured by the method.
- Truth_DMRs_Stage1Recoverable_FullyCapturedSeeds: Number of truth DMRs where all overlapping seed CpGs are captured by CMEnt-predicted DMRs, indicating ideal stage 1 capture.
- Seed_Capture_Rate_Pct_in_Truth: Percentage of seed CpGs that overlap truth DMRs that are also captured by CMEnt-predicted DMRs.
- Median_Seed_Capture_Pct_Per_Truth: For truth DMRs that have at least one overlapping seed CpG, the median percentage of those seed CpGs that are captured by CMEnt-predicted DMRs.
- Truth_DMRs_With_Missed_Seeds: Number of truth DMRs that have at least one overlapping seed CpG that is not captured by CMEnt-predicted DMRs.
