...
if (!requireNamespace("karyoploteR", quietly = TRUE)) {
    ...
    BiocManager::install("karyoploteR", update = FALSE, ask = FALSE)
}
library(karyoploteR)
...
We rank seeds by a combined score of p-value and effect size (delta beta) to prioritize biologically relevant candidates.
...
if (is.null(seed_locs) || nrow(seed_locs) == 0) {
    seed_locs <- data.frame(
        chr = sub(":.*$", "", sig_seeds$cpg_id),
        start = suppressWarnings(as.integer(sub("^[^:]+:([0-9]+).*$", "\\1", sig_seeds$cpg_id))),
        end = suppressWarnings(as.integer(sub("^[^:]+:([0-9]+).*$", "\\1", sig_seeds$cpg_id))),
        stringsAsFactors = FALSE
    )
    seed_locs <- seed_locs[!is.na(seed_locs$start) & nzchar(seed_locs$chr), , drop = FALSE]
}
...
### Genome-wide Seeds and DMRs Visualization

```{r seed_dmr_karyoplot, fig.width=14, fig.height=8}
seed_plot_gr <- GenomicRanges::makeGRangesFromDataFrame(
    seed_locs,
    seqnames.field = "chr",
    start.field = "start",
    end.field = "end",
...
all_plot_gr <- c(seed_plot_gr, truth_dmrs)
chr_lengths <- tapply(
    GenomicRanges::end(all_plot_gr),
    as.character(GenomicRanges::seqnames(all_plot_gr)),
    max,
    na.rm = TRUE
)
chr_lengths <- chr_lengths[is.finite(chr_lengths) & chr_lengths > 0]
plot_genome_df <- data.frame(
    chr = names(chr_lengths),
    start = 1L,
    end = as.integer(chr_lengths),
    stringsAsFactors = FALSE
)
plot_genome_df <- plot_genome_df[order(plot_genome_df$chr), , drop = FALSE]

kp <- karyoploteR::plotKaryotype(
    genome = plot_genome_df,
    chromosomes = plot_genome_df$chr,
    plot.type = 2
)
...
karyoploteR::kpAddLabels(kp, labels = "Seed density", r0 = 0.06, r1 = 0.46, cex = 0.8, label.margin = 0.02)
...
karyoploteR::kpAddLabels(kp, labels = "Ground-truth DMRs", r0 = 0.58, r1 = 0.95, cex = 0.8, label.margin = 0.02)
graphics::legend(
    "bottomleft",
    legend = c("Seed density", "Ground-truth DMRs"),
...
ord <- order(seed_df[[rank_col]], decreasing = TRUE, na.last = NA)
...
knitr::kable(
    top_seed_overlap,
    digits = 2,
    caption = sprintf("Ground-truth overlap enrichment among top-ranked seeds (%s)")
)
```
...
```{r hummingbird}
hummingbird_file <- file.path(output_dir, "dmrs.hummingbird.rds")
...
run_hummingbird_bsseq <- function(
    ...
    if (!requireNamespace("hummingbird", quietly = TRUE)) {
        warning("hummingbird not installed; returning empty set.")
        return(GenomicRanges::GRanges())
    }
    ...
    for (chr_name in chr_vec) {
        ...
        cat(sprintf(
            "hummingbird: %s (%s CpGs)\n",
            chr_name,
            format(length(chr_idx), big.mark = ",")
        ))
        ...
        chr_out <- tryCatch(
            {
                hb_fit <- hummingbird::hummingbirdEM(control = control_se, abnorm = case_se)
                ...
                dmrs_df <- hb_fit$DMRs
                if (is.null(dmrs_df) || nrow(dmrs_df) == 0) {
                    return(NULL)
                }
                if (!all(c("chromosome", "start", "end") %in% colnames(dmrs_df))) {
                    warning("hummingbird output missing chromosome/start/end columns; skipping chromosome.")
                    return(NULL)
                }
...
dmrs_in_truth_counts_df[, "DMRsegal_DMR_Count_P"] <- ifelse(dmrs_in_truth_counts_df$DMRsegal_DMR_Count > 0, 1, NA_integer_)
...
ggplot(dmrs_in_truth_counts_df, aes(x = Truth_DMR_Index, y = DMRsegal_DMR_Count, color = Truth_BP_Recall_Pct)) +
    geom_point(mapping = aes(size = DMRsegal_DMR_Count_P), alpha = 0.7, na.rm = TRUE) +
    geom_line(group = 1, color = "darkgray", alpha = 0.7) +
...
if (length(dmrs_segal) > 0 && "score" %in% names(mcols(dmrs_segal))) {
    dmrs_segal_ranked <- dmrs_segal[order(mcols(dmrs_segal)$score, decreasing = FALSE)]
    truth_reduced <- reduce(truth_dmrs)

    n_bins <- min(100, max(2, floor(length(dmrs_segal_ranked) / 5)))
    ...
    ggplot(
        ranking_quality,
        aes(x = Score_Bin, y = BP_Overlap_Pct)
    ) +
        geom_col(fill = "None", col = "black") +
        geom_line(group = 1, color = "darkgray", linewidth = 1) +
        theme_minimal() +
        scale_x_continuous(breaks = ranking_quality$Score_Bin) +
        scale_y_continuous(labels = scales::percent_format(scale = 1)) +
        labs(
            title = "DMRsegal Rank Quality",
            x = "Score Bin (1 = best ranks)",
