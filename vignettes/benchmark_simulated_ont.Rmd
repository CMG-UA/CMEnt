...
get_benchmark_file <- function(filename) {
    benchmark_file <- ""
    try(benchmark_file <- system.file("benchmarks", "simulated_ont", filename, package = "DMRsegal"), silent = TRUE)
    if (benchmark_file == "") {
        if ("inst" %in% dir()) {
            benchmark_file <- file.path("inst", "benchmarks", "simulated_ont", filename)
        } else if ("inst" %in% dir("../")) {
            benchmark_file <- file.path("..", "inst", "benchmarks", "simulated_ont", filename)
        } else {
            benchmark_file <- file.path("benchmarks", "simulated_ont", filename)
        }
        benchmark_file <- file.path("inst", "benchmarks", "simulated_ont", filename)
    }
...
all_plot_gr <- c(seed_plot_gr, truth_dmrs, GenomicRanges::granges(sim_bs))
chr_max <- tapply(
...
chr_max <- chr_max[is.finite(chr_max) & chr_max > 0]

# Prefer full chromosome lengths from a matching reference genome if available.
candidate_genomes <- c("hg38", "hg19")
candidate_scores <- rep(Inf, length(candidate_genomes))
names(candidate_scores) <- candidate_genomes
for (genome_name in candidate_genomes) {
    si <- tryCatch(GenomeInfoDb::Seqinfo(genome = genome_name), error = function(e) NULL)
    if (is.null(si)) {
        next
    }
    sl <- GenomeInfoDb::seqlengths(si)[names(chr_max)]
    if (all(!is.na(sl)) && all(chr_max <= sl)) {
        candidate_scores[genome_name] <- sum(as.numeric(sl) - as.numeric(chr_max))
    }
}

if (any(is.finite(candidate_scores))) {
    best_genome <- names(which.min(candidate_scores))[1]
    ref_chr_lengths <- GenomeInfoDb::seqlengths(
        GenomeInfoDb::Seqinfo(genome = best_genome)
    )[names(chr_max)]
    chr_lengths <- pmax(as.numeric(chr_max), as.numeric(ref_chr_lengths), na.rm = TRUE)
    names(chr_lengths) <- names(chr_max)
} else {
    chr_lengths <- chr_max
}
...
plot_params <- karyoploteR::getDefaultPlotParams(plot.type = 2)
plot_params$leftmargin <- 0.16
plot_params$rightmargin <- 0.03
...
karyoploteR::kpAddLabels(kp, labels = "Seed density", r0 = 0.06, r1 = 0.46, cex = 0.8, label.margin = 0.005)
...
karyoploteR::kpAddLabels(kp, labels = "Ground-truth DMRs", r0 = 0.58, r1 = 0.95, cex = 0.8, label.margin = 0.005)
graphics::legend(
    "bottomleft",
