...
This vignette describes the main configuration parameters available in CMEnt.
These parameters control input handling, region expansion, statistical testing,
annotation, scoring, and performance.
...
### `sample_group_col`
**Type:** Character  
**Required:** Yes, whenever `pheno` or a samplesheet is provided  
...
**Required:** When beta row names are array probe IDs and no explicit genomic locations are supplied  
**Description:** Type of methylation array platform used. Ignored when beta is provided as a BED file or explicit genomic locations are supplied.


### `genome`
**Type:** Character
**Options:** `"hg19"`, `"hg38"`, `"hs1"`, `"mm10"`, `"mm39"`
**Description:** Reference genome version. Required when array annotations or BED coordinates need an explicit genome. For DMR `GRanges` inputs, functions infer the genome from seqinfo when available.
