...
#' Preprocess EPICv2 Beta Values Using DMRcate
#'
#' @description Preprocesses EPICv2 array data using DMRcate's cpg.annotate() function.
#' This handles cross-hybridizing probe remapping and replicate probe filtering.
#'
#' EPICv2 arrays contain probes that cross-hybridize to locations different from their
#' intended targets. Additionally, after remapping, multiple probes may target the same
#' CpG site. This function uses DMRcate's methodology to address both issues.
#'
#' @param beta Matrix of beta values with EPICv2 probe IDs as rownames.
#'   Probe IDs should have 15 characters (e.g., cg00000029_TC21).
#' @param epicv2Remap Logical. Remap cross-hybridizing probes to their likely targets
#'   based on whole-genome bisulfite sequencing (WGBS) evidence. Default TRUE.
#'   See DMRcate::cpg.annotate() for details.
#' @param epicv2Filter Character. Strategy for handling replicate probes that map to
#'   the same CpG site after remapping:
#'   \itemize{
#'     \item "mean": Average beta values across replicates (default)
#'     \item "sensitivity": Select probe most sensitive to methylation changes
#'     \item "precision": Select probe with best measurement precision
#'     \item "random": Randomly select one probe per position
#'   }
#'
#' @return List with:
#'   \item{beta}{Preprocessed beta matrix with filtered/aggregated probes}
#'   \item{coords}{Data frame with chr, start, end for each probe (remapped if applicable)}
#'   \item{probe_mapping}{Data frame mapping original probe IDs to retention status}
#'
#' @details This function uses DMRcate (Peters et al.) for EPICv2-specific preprocessing.
#'   DMRcate will be installed automatically if not present.
#'
#'   Credit: DMRcate package by Tim J. Peters et al.
#'
#' @references
#' Peters, T.J., Meyer, B., Ryan, L., et al. (2024). Characterisation and reproducibility
...
#' @examples
#' \dontrun{
#' # Preprocess EPICv2 beta values
#' result <- preprocessEPICv2(epicv2_beta, epicv2Remap = TRUE, epicv2Filter = "mean")
#'
#' # Access preprocessed data
#' preprocessed_beta <- result$beta
#' remapped_coords <- result$coords
#' }
#'
#' @export
preprocessEPICv2 <- function(beta,
                              epicv2Remap = TRUE,
                              epicv2Filter = c("mean", "sensitivity", "precision", "random")) {
    epicv2Filter <- match.arg(epicv2Filter)

    # Install DMRcate on-the-fly if needed
    if (!requireNamespace("DMRcate", quietly = TRUE)) {
        .log_info("Installing DMRcate package required for EPICv2 preprocessing...")
        if (!requireNamespace("BiocManager", quietly = TRUE)) {
    ...
    # Create minimal design matrix for cpg.annotate (required but not used for preprocessing)
    dummy_design <- cbind(Intercept = 1, Group = rep(c(0, 1), length.out = ncol(beta)))

    # Use DMRcate's cpg.annotate for EPICv2 preprocessing
    .log_step("Preprocessing EPICv2 data using DMRcate::cpg.annotate()...", level = 1)
    cpg_obj <- DMRcate::cpg.annotate(
        datatype = "array",
        object = beta,
        what = "Beta",
        arraytype = "EPICv2",
    ...
    # Extract preprocessed data from CpGannotated object
    preprocessed_betas <- cpg_obj@betas
    ranges <- cpg_obj@ranges

    # Build coordinate data frame matching DMRsegal's expected format
    coords <- data.frame(
        chr = as.character(GenomicRanges::seqnames(ranges)),
        start = GenomicRanges::start(ranges),
        end = GenomicRanges::end(ranges),
        row.names = names(ranges)
    )

    # Track probe mapping for transparency
    original_probes <- rownames(beta)
    final_probes <- rownames(preprocessed_betas)
    probe_mapping <- data.frame(
        original = original_probes,
        retained = original_probes %in% final_probes
    )

    .log_success(sprintf("EPICv2 preprocessing complete: %d -> %d probes",
                         nrow(beta), nrow(preprocessed_betas)), level = 1)

    list(
        beta = preprocessed_betas,
        coords = coords,
